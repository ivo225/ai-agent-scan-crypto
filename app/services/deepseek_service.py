...
async def get_deepseek_analysis(
    ...
    """
    Uses DeepSeek API to analyze cryptocurrency data, including technical indicators,
    ...
    Args:
        ...
        sentiment_data: Optional dictionary containing sentiment/news data.
    ...
    if market_context:
        ...
        if gm_data:
            ...
            prompt_lines.append(f"- BTC Dominance: {f'{btc_dom:.2f}%' if btc_dom is not None else 'N/A'}")
    ...
    if sentiment_data and sentiment_data.get("top_posts"):
        ...
        for post in sentiment_data["top_posts"]:
         ...
         prompt_lines.append(f"\nRecent News Count: {sentiment_data.get('count', 0)}")
    ...
    prompt_lines.append("\nBased on all the above data (individual coin market/sentiment/technicals, overall market context, confidence assessment):") # Added market context
    prompt_lines.append("1. Provide a detailed short-term price prediction (1-4 weeks) with potential price targets (low/high range) and key support/resistance levels. Consider the overall market context (F&G, global change) and calibrate certainty based on the confidence score.")
    prompt_lines.append("2. Provide a detailed long-term price prediction (3-12 months) with potential price targets (low/high range). Consider the overall market context and calibrate certainty based on the confidence score.")
    prompt_lines.append("3. Briefly explain how the technical indicators AND the overall market context support your predictions.")
    prompt_lines.append("4. Analyze how the current news sentiment might impact price action (consider if priced in, potential catalysts/reversals).")
    prompt_lines.append("5. Suggest potential entry price points based on the combined analysis.")
    prompt_lines.append("6. Summarize the overall outlook and confidence level (Low/Medium/High).")
    ...
    system_prompt = (
        ...
        "sentiment, and technical indicators (RSI, MACD, SMA, Bollinger Bands). Provide structured, "
        "actionable insights including specific price targets, support/resistance levels, and time horizons. "
        "Explain the reasoning based on the provided data. Be concise and do not ask follow-up questions."
        ...
        "\n- SMA 50: Price vs SMA 50 shows short/medium-term trend." # Updated SMA guide
        ...
        "\n- Consider news source, votes, and potential market reaction."
        "\n- Evaluate if news is already priced in or could be a future catalyst."
        "\n- Note sentiment/price divergences."
